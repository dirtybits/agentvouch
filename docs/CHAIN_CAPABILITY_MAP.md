<a id="agentvouch-chain-capability-map"></a>

# AgentVouch Features by network
...
>
> This table separates features written in code from features installed and usable on each network.
>
...
protocol receipt, author's sales earnings, backers' revenue shares, or dispute state. Optional
...
| Self-stake       | Withdraws an eligible author's backing deposit.                                       | `withdraw_author_bond`          | `LIVE_DEVNET`    | `withdrawAuthorBond`                                                                                                      | `MERGED_SOURCE`  | `LIVE_SEPOLIA_PRE_A1` | Both enforce exposure locks; Base currently has only its pre-A1 report lock.                                                                                             |
...
| Slash            | Permissionlessly settles eligible deducting backers' deposited USDC in bounded pages. | `slash_dispute_vouches`         | `LIVE_DEVNET`    | `slashPaidPurchaseReportVouches`                                                                                          | `MERGED_SOURCE`  | `NOT_DEPLOYED`        | Solana processes listing-linked positions; Base processes author-wide vouches against the snapshotted active stake.                                                      |
...
| Earn             | Withdraws author's sales earnings for a listing revision.                             | `withdraw_author_proceeds`      | `LIVE_DEVNET`    | `withdrawAuthorProceeds`                                                                                                  | `MERGED_SOURCE`  | `LIVE_SEPOLIA_PRE_A1` | Both preserve revision-scoped author's sales earnings.                                                                                                                   |
| Refund           | Creates and closes bounded restitution liabilities for an upheld paid dispute.        | `create_refund_pool`            | `LIVE_DEVNET`    | `resolvePaidPurchaseReport`, `slashPaidPurchaseReportVouches`, `closePaidPurchaseReportCredit`, `claimRestitutionReserve` | `MERGED_SOURCE`  | `NOT_DEPLOYED`        | Base funds credit during uphold/final voucher processing, closes expired credit separately, and exposes pull-only excess to the configured recipient of remaining funds. |
...
## Human Base wallet execution paths

This section records web-client execution support separately from the protocol surface and from
deployment status. It does not upgrade any `NOT_DEPLOYED` selector in the table above.

| Wallet path           | Current source support                                                                                                                                                                 | Execution model                                                                                                                              | Live evidence                                                                                                                                                                             |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Coinbase Smart Wallet | Connect/auth, registration, listing lifecycle, author's backing deposit, vouch/revoke, purchase, voucher-revenue claim, author-proceeds withdrawal, paid-purchase report/credit claim. | Sponsored ERC-4337 UserOperations; approvals and protocol calls may be batched.                                                              | Existing Base Sepolia passkey smokes cover the historical candidate surface; A1 actions remain gated by the fresh-deployment plan.                                                        |
| MetaMask injected     | Connect/auth, registration, listing lifecycle, author's backing deposit, vouch/revoke, purchase, voucher-revenue claim, author-proceeds withdrawal, paid-purchase report/credit claim. | User-paid Base Sepolia EOA transactions; USDC approvals are exact and sequential. ERC-7702/EIP-5792 capability is detected but not executed. | Purchase, entitlement, and signed download were live-smoked 2026-07-07. Registration, listing, trust writes, and claims still require a fresh human smoke before being described as live. |

Both paths remain pinned to `eip155:84532`; neither enables Base mainnet. Wallet parity applies to
an identity controlled by that wallet. A MetaMask EOA cannot manage listings, stake, or proceeds
owned by an existing Coinbase smart-account address without a separate on-chain authority-transfer
design.

<a id="merged-voucher-slashing-a1-deployment-gap"></a>

## Merged Deducting backers' deposited USDC (A1) Deployment Gap
...
This is a source claim only. The live Base Sepolia address remains the pre-A1 `base-v1-candidate` and does not route the contract functions for paid-purchase reports. PR #106 shipped the paid-report wallet/UI writes behind `NEXT_PUBLIC_BASE_PAID_PURCHASE_REPORTS_ENABLED`; approval and activation, a fresh linked deployment, check against the deployed system, custody/operations inputs, and external review or explicit human acceptance remain gated by `.agents/plans/base-paid-report-activation-sepolia.plan.md`.
