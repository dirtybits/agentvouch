...
Do not proceed until the activation plan explicitly approves candidate verification.
...
GitHub workflow currently requests Foundry `stable`. Candidate verification must use Foundry
`v1.7.1`, matching
...
### Read-only Deployment Preflight, Monitor, and Isolated-Lifecycle-Test Readiness
...
After the paused deployment (Base Sepolia Deployment Gate B) and the founder decision sheet are
complete, `gate-c-readiness` adds two required public inputs:
...
invokes a signer, simulates a write, submits a transaction, or authorizes the isolated lifecycle
test. A mismatch produces `BLOCKED`, records exact blockers, and exits nonzero.

The `gate-c-readiness` mode derives the candidate commit from a clean local Git `HEAD`. It proves the
declared facade creation boundary by reading code at the deployment block and the preceding block.
It rescans the complete on-chain event history from that boundary. It ignores cached event/checkpoint
...
Only broadcast after a human approves the exact deploy-without-initialization action:
...
Only broadcast after a human separately approves the verify-pause-configure-and-transfer action:
...
Approve preview activation only after the isolated lifecycle test. Shared Sepolia promotion requires
another explicit decision after the preview rollback test.
...
deployment address and its on-chain pause state remain authoritative. Preview and shared testnet
activation must update and verify the deployment pointer, paymaster policy, and this flag as separate
recorded changes. Merging this code leaves the flag off by default.
