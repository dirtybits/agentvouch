...
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------- | ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A1 — Base voucher slashing                   | Ship the clean-break PaidPurchaseReport mechanism so an eligible buyer receipt can trigger centrally adjudicated, author-bond-first and author-wide voucher slashing with bounded buyer credit.                                 | Yes                                | Yes                                              | **Local implementation complete; launch blocked.** The linked `base-v1-a1` candidate is size-feasible (23,487-byte facade; 5,939-byte library), 116 Forge tests pass, and a 31-transaction ephemeral Anvil rehearsal completed the full paid-report settlement flow. An internal executable-diff review covered 50/50 surfaces with no reportable findings. The separately approved paid-report write client, fresh Base Sepolia deployment/live smoke, external security pass or explicit human acceptance, and custody/operations inputs remain open. | `.agents/plans/base-a1-voucher-slashing-port.plan.md`; `npm run verify:base-size`                                                                                                                      |
...
- **2026-07-13:** The clean-break Base A1 candidate reached local implementation completion: contract
  and consumers synchronized, EIP-170 and soft-size gates passed, 116 Forge tests and the complete
  local Anvil lifecycle passed, and an internal executable-diff review found no reportable issues.
  Launch remains blocked on the separately approved paid-report client, fresh Base Sepolia evidence,
  external review or human acceptance, custody, monitoring, and exposure-policy gates.
