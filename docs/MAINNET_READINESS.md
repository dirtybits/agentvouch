...
>
> This is the list of checks and approvals required before a real-money launch. A requirement is not satisfied merely because its code exists.
>
...
- **First small Base release with founder-reviewed reports and limits on customer money:** founder/admin report resolution is acceptable only under
  the [Minimum buyer-protection requirements](#launch-trust-bar-chain-agnostic-2026-07-06), with explicit limits on money at risk.
- **Full trust-minimized Base mainnet:** adds governed review of buyer reports (requirement A2) before the launch
  can be called trust-minimized.

<a id="base-mainnet-gate-table"></a>

## Base real-money launch requirements

This table is **Base-track only**. Solana's endorser stake deductions (requirement A1) and emergency
pause (requirement A3) are already live-tested on devnet and are kept
...
### Base gate terminology

This document uses two different sequences. Always include the topic when you refer to an item.

- **Base Sepolia Deployment Gates A–D** control deployment stages. They cover candidate verification,
  paused deployment, isolated tests of buyer reports and credit claims, and testnet activation.
- **AgentVouch Protocol Requirements A1–A5** define product and security behavior across chains.
  The descriptive names below match `docs/ROADMAP.md`. This table reports the Base implementation
  status.

Requirement A1 means **deduct USDC deposited as backing after upheld buyer reports**. An upheld report is one
the resolver decides is valid. The protocol calls deductions from endorsers' stakes "voucher
slashing". On Base, settlement uses the author's bond first, then eligible endorsers' stakes,
to fund a collateral-limited amount owed to the buyer. This does not guarantee a full refund.

Do not use a bare label such as “Gate A” or “A1” in reader-facing text. The two sequences are not one
continuous sequence. Names clarified on 2026-09-10; this wording update changes no Check and approval status,
policy, or approval.

| Base status for protocol requirement                                                    | Required outcome                                                                                                                                                                                                                                                                                                                                                     | Base-alpha blocking?                                    | Full-mainnet blocking?                           | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Evidence (link)                                                                                                                                                                                                                                     |
| --------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AgentVouch Requirement A1 - Deduct USDC deposited as backing After Upheld Buyer Reports | Ship the clean-break PaidPurchaseReport mechanism so an eligible buyer receipt can trigger centrally adjudicated, author-bond-first and author-wide deducting backers' deposited USDC with bounded amount owed to the buyer.                                                                                                                                         | Yes                                                     | Yes                                              | **Merged implementation complete; launch blocked.** PR #102 merged the linked `base-v1-a1` source. It is size-feasible (23,487-byte facade; 5,939-byte library), 121 Forge tests pass, and a 31-transaction ephemeral Anvil practice run completed the full paid-report settlement flow. An internal executable-diff review covered 50/50 surfaces with no reportable findings. PR #106 since shipped the wallet functions for submitting paid-purchase reports (`basePaidPurchaseReports.ts`, `baseWallet.ts`, `baseInjectedWallet.ts`), flag-gated off behind `NEXT_PUBLIC_BASE_PAID_PURCHASE_REPORTS_ENABLED`; what remains is approval and activation, not implementation. Fresh paused Base Sepolia deployment/check against the deployed system, external security pass or explicit human acceptance, and custody/operations inputs remain open. | `.agents/plans/base-a1-voucher-slashing-port.plan.md`; `.agents/plans/base-paid-report-activation-sepolia.plan.md`; `docs/BASE_SEPOLIA_A1_STATE.md`; `npm run verify:base-size`                                                                     |
| AgentVouch Requirement A2 - Replace Founder-Only Report Decisions With Governed Review  | Replace founder-only resolution with two-phase/timelocked/report review with approval and waiting-period rules and reserve-aware treasury rules.                                                                                                                                                                                                                     | No, if the Minimum buyer-protection requirements passes | Yes                                              | **Deferred for alpha; required for full trust-minimized mainnet.** Governed dispute-resolution invariants still constrain the alpha mechanism: buyer-first routing, capped rewards, snapshots, and zero-refund branches.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `.agents/plans/a2-dispute-governance-v1.plan.md`; `.agents/plans/a2-s*.plan.md`; [governed dispute-resolution findings](#a2-extra-review-findings-2026-06-17); [Minimum buyer-protection requirements](#launch-trust-bar-chain-agnostic-2026-07-06) |
| AgentVouch Requirement A3 - Pause New Activity While Keeping Claims Open                | Keep `setPaused` live on the Base v1 contract, put `PAUSE_ROLE` under approved custody, and prove pause blocks new exposure while refund/claim exits remain open.                                                                                                                                                                                                    | Yes                                                     | Yes                                              | **Partial.** Pause semantics exist in the EVM contract family; production policy for holding and using signing keys and release-candidate smoke are still open.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `.agents/plans/a3-emergency-pause.plan.md`; `.agents/plans/base-port-chain-adapter-phase-9.plan.md`; [Authority Policy](#authority-policy); [Incident Response](#incident-response)                                                                 |
| AgentVouch Requirement A4 - Limit Amounts owed to buyers and Assign Unused Funds        | Keep amount owed to the buyer collateral-limited with no guaranteed extra refund funding from AgentVouch; route slash excess, rejected/dismissed bonds, and expired amount owed to the buyer to a separate, fixed recipient who must request payment.                                                                                                                | Yes                                                     | Yes                                              | **Policy locked; custody pending.** Economics are fixed in the deducting backers' deposited USDC plan, but the recipient, custody approval, monitoring, and live pull evidence remain deployment requirements.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | `.agents/plans/base-a1-voucher-slashing-port.plan.md`                                                                                                                                                                                               |
| AgentVouch Requirement A5 - Test Complete Flows and Review Contract Security            | Run the full local gate suite, forge suite, live Base smokes, internal review, and one external security pass or explicit human-recorded acceptance for the complete v1 mechanism.                                                                                                                                                                                   | Yes                                                     | Yes                                              | **Partial.** The full local gates and internal 50/50 executable-diff review completed with no reportable findings. Live Base smoke and one external security pass or explicit human-recorded acceptance remain required.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | [Security Review](#security-review); `.agents/plans/base-a1-voucher-slashing-port.plan.md`; `.agents/plans/base-port-chain-adapter-phase-9.plan.md`                                                                                                 |
| Money limits for the first small release                                                | Define the cap that makes founder-operated alpha acceptable: max listing price, aggregate protocol exposure/TVL, and sponsor-paid network-fee spend cap.                                                                                                                                                                                                             | Yes                                                     | No, once replaced by full production risk policy | **DECISION NEEDED (founder).** Do not invent numbers in implementation plans.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | [Minimum buyer-protection requirements](#launch-trust-bar-chain-agnostic-2026-07-06); [Platform Concentration Risk](#platform-concentration-risk-coinbase-2026-07-06)                                                                               |
...
| Card payment method activation (Stripe)                                                 | Before any real card sale: choose the plan for moving beyond the first card-payment trial, record the decision about which business is responsible for the card sale, define the author payout process (Connect vs. manual vs. fiat→USDC) including tax/KYC, and verify the external Vercel WAF rule. Live Stripe keys must stay behind an explicit acknowledgement. | Yes                                                     | Yes                                              | **DECISION NEEDED (founder); source hard-disabled.** Checkout, webhook reconciliation, terminal refund/dispute markers, immutable pilot accounting, buyer/skill scope, and atomic limits on money at risk are implemented locally. Live session creation and fulfillment cannot be enabled by environment values until disposable-database practice run, payout/tax/support decisions, WAF and monitoring proof, and explicit activation review land.                                                                                                                                                                                                                                                                                                                                                                                                  | [Card payments](#card--fiat-rail-stripe-2026-07-26); `.agents/plans/stripe-live-limited-pilot.plan.md`; `docs/STRIPE_FEASIBILITY.md`; `docs/STRIPE_MPP_POLICY.md`; `docs/STRIPE_TEST_MODE_ROLLOUT.md`                                               |
| Card buyer recourse disclosure                                                          | Card buyers receive an download permission recorded by the app rather than the blockchain with no protocol recourse. Trust surfaces, checkout copy, and public docs must state this plainly before any card sale settles.                                                                                                                                            | Yes                                                     | Yes                                              | **Implemented in source; deployment proof pending.** The card button requires acknowledgement of versioned recourse copy, carries the version in Stripe metadata, and links to `/docs#card-checkout-recourse`; `web/public/skill.md` states the same boundary. A production deploy and browser verification are still required before this row is green.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | [Card payments](#card--fiat-rail-stripe-2026-07-26); [Minimum buyer-protection requirements](#launch-trust-bar-chain-agnostic-2026-07-06)                                                                                                           |
...
- **2026-06-09:** Solana readiness re-read confirmed the active devnet program identity and locked
  the A1 deducting backers' deposited USDC design in `.agents/plans/a1-voucher-slashing.plan.md`.
- **2026-06-10:** Solana A1 deducting backers' deposited USDC deployed and verified on devnet.
...
- **2026-07-06:** Minimum buyer-protection requirements recorded founder-operated alpha as acceptable only under five
  conditions; Base A1 deducting backers' deposited USDC was promoted to a Phase 9b-2 launch gate.
...
- **2026-07-26:** Readiness re-read against `main` at `f2d808e`. Three Base rows were stale (Forge
  ...
  wallet functions for submitting paid-purchase reports shipped in #106 and is flag-gated, so only approval and activation
  remain). The Stripe card payment method — merged 2026-07-14 through 2026-07-18 — had no representation in
  this document at all; it is now gated by two table rows and the
  [Card payments](#card--fiat-rail-stripe-2026-07-26) section. A live-key guard was added in
  code because the test-mode constraint had existed only as prose.

<a id="launch-trust-bar-chain-agnostic-2026-07-06"></a>

## Minimum buyer-protection requirements (chain-agnostic, 2026-07-06)
...
1. **Enforced voucher downside exists on the launch chain.** Vouching must not be reward-only:
   upheld paid-listing reports/disputes slash linked backing deposit, not just the author's backing deposit.
...
<a id="card--fiat-rail-stripe-2026-07-26"></a>

### Card payments (Stripe, 2026-07-26)
...
and `NEXT_PUBLIC_STRIPE_CHECKOUT_ENABLED=true`. Every live key, regardless of deployment environment,
additionally requires `AGENTVOUCH_STRIPE_LIVE_MODE_ENABLED=true` and
`AGENTVOUCH_STRIPE_EDGE_RATE_LIMIT_READY=true`; production deployments retain that acknowledgement
requirement for test keys as well. The account-scoped path also requires buyer auth plus
`AGENTVOUCH_BUYER_CARD_ACCESS_ENABLED`. Live Checkout Session creation remains source-disabled until
the implementation-readiness constant is deliberately changed after founder decisions,
disposable-database practice run, WAF/monitoring proof, and explicit activation review land.
Production runs Clerk buyer auth with every card and checkout flag explicitly false (see
`.agents/plans/walletless-production-auth-rollout.plan.md`). Only a test-mode preview smoke has run.

**Live-key guard (added 2026-07-26).** The test-mode constraint previously existed only as prose in
`docs/STRIPE_TEST_MODE_ROLLOUT.md`, and no code inspected the key prefix. At that time the edge-rate-limit
acknowledgement applied only when `VERCEL_ENV === "production"`, so a live key on a preview deployment
could have started real commerce with no further gate. `detectStripeKeyMode` now classifies the
configured key, an unrecognized prefix fails closed, and the edge-rate-limit acknowledgement is now
required when the deployment is production **or** the key is live. Every `live` key additionally
requires the explicit live-mode flag. The webhook refuses to grant access on an event whose
`livemode` contradicts the configured key (recorded as needs-review, not a retryable error). Refund
and dispute events still revoke access before that guard; terminally acknowledging them without
revocation would leave access active and prevent Stripe redelivery.
...
protocol receipt, no author's sales earnings, and no backers' revenue shares. Stripe receipts are written with
`evm_purchase_id = NULL` and `legacy_refund_eligible = FALSE`, which makes a card buyer
**ineligible by construction** to file a bonded paid report, trigger deducting backers' deposited USDC, or claim
amount owed to the buyer. An on-chain buyer gets all of it. A card buyer's only recourse is a chargeback
...
1. **Plan for moving beyond the first card-payment trial** — card on-ramp to protocol settlement, parallel MPP marketplace, or limited
   early-sales rail (`docs/STRIPE_FEASIBILITY.md`). The limited rail is the current pilot proposal,
   but founder acceptance is not yet recorded.
...
5. **Limited-pilot controls** now fail closed without opaque buyer-account and skill allowlists,
   unit/gross/completed/concurrent caps, reservation TTL, reconciliation SLA, and an additive
   amount/fee/net ledger. They remain un-rehearsed against a disposable copy of the intended
   production database, and the source activation stop remains closed.
6. **Edge rate limiting** is an in-memory per-instance limiter plus an honor-system boolean; the
   Vercel Firewall/WAF rule itself is unbuilt and unpublished.
7. **`marketplace_access_grants` is not self-bootstrapping** (unlike `stripe_webhook_outcomes`); it
   is created by `web/scripts/walletless-buyer-migration.ts`. An account-path webhook against an
   unmigrated database will 500.

**Disclosure source state (2026-07-31):** checkout now requires an identity-bound checkbox covering no
protocol receipt, author's sales earnings, backers' revenue shares, paid Report/slashing path, or amount owed to the buyer.
The accepted disclosure version is sent to Checkout Session and PaymentIntent metadata. Live
fulfillment remains entirely source-disabled pending the external/founder gates. Reservation-bound
validation is implemented but dormant. The same copy is in `/docs#card-checkout-recourse` and
`web/public/skill.md`. This is source evidence only until a production deployment and browser smoke
pass.
...
<a id="a2-extra-review-findings-2026-06-17"></a>

### AgentVouch Protocol Requirement A2 — Additional Review Findings (2026-06-17)
...
9. **Author-bond exposure should be serialized in A2 v1.** The author's backing deposit is one profile-level collateral pot. Until the protocol has aggregate exposure accounting or per-dispute reserves, A2 should reject new author-bond-exposing disputes while `author_profile.open_author_disputes > 0`.
...
- purchase, vouch, author's backing deposit, dispute, and claim events
...
- free-skill report uses author's backing deposit exposure
- paid-skill report uses author's backing deposit first, then linked vouchers where applicable
...
- Anchor test-gap note (as of 2026-06-24): `update_skill_listing`, `remove_skill_listing`, `close_skill_listing`, and `initialize_listing_settlement` are implemented but not covered by Anchor tests; extend `tests/agentvouch-usdc-marketplace.ts` / `tests/agentvouch-usdc-disputes.ts` before any Solana RC, and add at least one API ↔ on-chain integration test proving entitlement/refund bookkeeping matches on-chain truth (the web suite is mocked).
