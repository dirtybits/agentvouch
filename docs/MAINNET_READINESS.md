...
1. **Voucher slashing is not implemented.** The `AuthorBondThenVouchers` liability scope is recorded, but no instruction ever slashes voucher stake: `resolve_author_dispute.rs` only calls `slash_author_bond_if_present`; `author_dispute.voucher_slashed_usdc_micros` is set to `0` at open and never recomputed; `VouchStatus::Slashed` (`state/vouch.rs`) is never assigned; `AuthorDisputeVouchLink` (`state/author_dispute_vouch_link.rs`) is defined but never created. Net: **vouching is reward-only with zero downside** — the stake-backed-reputation thesis is not enforced on-chain. *Fix:* implement voucher slashing (debit voucher vaults, set `VouchStatus::Slashed`) and keep the active-dispute lock on `revoke_vouch`.
   ...
   > **Status (2026-06-09): implemented in source, pending devnet redeploy.** Voucher slashing is built and tested on the A1 branch: upheld paid disputes park in `SlashingVouchers` and a permissionless `slash_dispute_vouches` instruction settles linked positions in pages of ≤ 4 (5 remaining accounts per position; a 4-position page measured 31 accounts, inside the tx limit). Slashed funds are ring-fenced in `ListingSettlement.slashed_deposit_usdc_micros` and exit only through `create_refund_pool`. Full Anchor suite green (29 tests incl. 8 slashing tests: multi-page, dodge/rotation blocks, double-crank, stale-position skip-settle, residual reclaim, ring-fence, slashed-only pool, reward-vault solvency). Remaining: devnet clean-break redeploy (new program ID — `SkillListing`/`ListingSettlement` layouts changed) per the AGENTS.md runbook, then update `web/public/skill.md` to describe live slashing. This item stops blocking once deployed.
   >
