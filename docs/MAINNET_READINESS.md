...
the implementation-readiness constant is deliberately changed after the remaining controls land.
...
5. **Limited-pilot scope** now fails closed for live keys without an explicit skill UUID allowlist
   and per-charge ceiling, but there is no durable amount/fee/net ledger or atomic aggregate GMV /
   completed-payment cap. A global flag is not a sufficient limited-pilot control.
...
fulfillment is entirely source-disabled until reservation-bound validation lands. The same copy is
in `/docs#card-checkout-recourse` and `web/public/skill.md`. This is source evidence only until a
production deployment and browser smoke pass.
