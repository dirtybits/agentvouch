...
todos:
  ...
  - id: verify-render
    ...
    status: pending
  - id: verify-gates
    content: "web typecheck + lint + vitest green; confirm viem stays out of the client bundle (still dynamically imported, BaseAdapter reads server-side only). One commit/PR off feat/base-port-chain-adapter."
    status: pending
isProject: false
---
...
4. **Hydrate** (`fetch-base-listings`): implement `fetchBaseChainListings()` — select `eip155:%` rows,
   `getAdapter(ctx).fetchSkillListing(evm_listing_id)` each, map to `ChainSkillRow` (same shape
   `fetchOnChainListings` returns: `name`, `description`, `author_pubkey`, `price_usdc_micros`,
   `on_chain_address`→use `evm_listing_id`, etc.). Skip rows without `evm_listing_id`.
...
- **Gates:** `cd web && npm run typecheck && npm run lint && npm test` green. (Markdown is not in
