...
Post-merge code-review fix (2026-06-10, against committed branch):

- **R6 — listing-deletion brick (critical).** R1's freeze covers link/unlink/update/settlement-init but missed `remove_skill_listing` and `close_skill_listing`, neither of which had a dispute guard. An author could `remove` (no guard) then `close` (deletes the `SkillListing` PDA via `close = author`) while parked in `SlashingVouchers`; `slash_dispute_vouches` and `create_refund_pool` then can't deserialize the listing → dispute stuck forever, `open_author_disputes` never decrements, every linked voucher's `revoke_vouch` locked permanently. Fix: `require!(!skill_listing.is_dispute_locked())` in both handlers (the `close` constraint defers deletion to instruction exit, so a handler `require!` aborts first). Free listings are unaffected — `open_author_dispute` only sets the lock on the paid `AuthorBondThenVouchers` path. Tests added: remove/close blocked while locked then succeed after `create_refund_pool` clears the lock; link/unlink blocked in the `SlashingVouchers` window (not just `Open`); free-listing `AuthorBondOnly` dispute resolves through the bond-only terminal path without parking.
