...
- **2026-07-17:** The existing Base-v1 x402 regression was re-run safely against the isolated preview and live Base Sepolia candidate `0x5992…B7d1`. `/api/x402/supported` advertised `eip155:84532`, native Circle USDC, and EIP-3009; anonymous raw access for fixture `3224046a-d1cb-4831-a552-f5319c92bb10` returned a valid `402` for listing `0xe2f649…19481`; `PROTOCOL_VERSION` returned `base-v1-candidate`; settlement transaction `0x4acc49…e0a7d` still had status `0x1`; and a fresh signed re-download by buyer `0xAf1c…d2DB` returned `200` with matching content. Isolated Neon still contains exactly one chain-qualified receipt and one entitlement for purchase `0x677939…eedd`, `1_000_000` micros, revision `1`. No fresh settlement was broadcast because the dedicated relayer key is unavailable and the deployer was deliberately not substituted. This does not lift Base mainnet gates.
