...
const PROGRAM_ID = new web3.PublicKey('AGNtBjLEHFnssPzQjZJnnqiaUgtkaxj4fFaWoKD6yVdg');
const USDC = new web3.PublicKey('4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU');
const FEE_PAYER = new web3.PublicKey('89CGD862LLSPDHef7mLg78vtiwLy19mwMfH3UePurd9n'); // Kora sponsor
...
const loadKeypair = (p) => web3.Keypair.fromSecretKey(Uint8Array.from(JSON.parse(fs.readFileSync(p))));
...
function buildTx(blockhash, authority, agentProfile, reimburse, metadataUri) {
  ...
  tx.add(
    createTransferCheckedInstruction(authorityAta, USDC, SPONSOR_USDC_ATA, authority.publicKey, reimburse, 6),
  );
...
(async () => {
  ...
  if (exists) throw new Error(`agent_profile ${agentProfile.toBase58()} already exists — use a fresh authority to prove rent sponsorship`);
  const solBefore = await conn.getBalance(authority.publicKey);
  const usdcBefore = (await conn.getTokenAccountBalance(authorityAta)).value.amount;
  ...
  const est = await rpc('estimateTransactionFee', {
    transaction: b64(buildTx(blockhash, authority, agentProfile, 1, metadataUri)),
  ...
  console.log(`estimate: fee_in_lamports=${est.result.fee_in_lamports} fee_in_token=${est.result.fee_in_token}`);
  console.log(`user reimburses ${reimburse} micro-USDC (${(reimburse / 1e6).toFixed(4)} USDC)\n`);

  ({ blockhash } = await conn.getLatestBlockhash());
  const signed = await rpc('signTransaction', {
    transaction: b64(buildTx(blockhash, authority, agentProfile, reimburse, metadataUri)),
    sig_verify: false,
  });
  if (!signed.result) throw new Error('signTransaction REJECTED -> ' + signed.error?.message);
  console.log('signTransaction -> SIGNED by Kora');

  const sig = await conn.sendRawTransaction(Buffer.from(signed.result.signed_transaction, 'base64'));
  ...
  const usdcAfter = (await conn.getTokenAccountBalance(authorityAta)).value.amount;
  ...
  console.log(`agent_profile created: ${!!profile}, owner=${profile?.owner.toBase58()}, rent=${profile?.lamports} lamports (paid by sponsor)`);
  console.log(
