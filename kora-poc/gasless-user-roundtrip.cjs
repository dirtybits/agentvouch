...
function loadKeypair(path) {
  return web3.Keypair.fromSecretKey(
    Uint8Array.from(JSON.parse(fs.readFileSync(path)))
  );
}
...
(async () => {
  ...
  const est = await rpc("estimateTransactionFee", {
    transaction: b64(
      buildTx(blockhash, buyer, web3.Keypair.generate(), rentLamports, 1)
    ),
  ...
  console.log(
    `buyer will reimburse ${reimburse} micro-USDC (${(reimburse / 1e6).toFixed(
      4
    )} USDC)\n`
  );
  ...
  const signed = await rpc("signTransaction", {
    transaction: b64(
      buildTx(blockhash, buyer, newAcct, rentLamports, reimburse)
    ),
    sig_verify: false,
  });
  if (!signed.result)
  ...
  console.log(
    "signTransaction -> SIGNED by Kora (sponsor added fee-payer signature)"
  );
  ...
  console.log(
    `  SOL delta:  ${
      solAfter - solBefore
    } lamports  (must be 0 -> user paid no gas/rent)`
  );
  console.log(
    `  USDC delta: ${usdcAfter - usdcBefore} micros   (the reimbursement)`
  );
  console.log(
    `new account funded by sponsor: ${newAcctLamports} lamports rent`
  );
  console.log(
    solAfter === solBefore
