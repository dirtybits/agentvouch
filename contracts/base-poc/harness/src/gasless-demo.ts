...
async function main() {
  ...
  if (ROLES.some((r) => !pks[r])) {
    console.log("No owner keys set. Generating a persistent set — save these to .env:\n");
    for (const r of ROLES) {
      const pk = generatePrivateKey();
      const sa = await smartAccountFor(pk);
      console.log(`  ${r.toUpperCase()}_OWNER_PK=${pk}`);
      console.log(`    -> smart account (fund with test USDC): ${sa.address}\n`);
    }
    console.log(
  ...
  const bundlerFor = (account: SmartAccount) =>
    createBundlerClient({ account, client: publicClient, transport: http(cdpUrl), paymaster: true });
  const bundlers = {
  ...
  const usdcBalance = (a: Address) =>
    publicClient.readContract({ address: usdc, abi: erc20Abi, functionName: "balanceOf", args: [a] });
  ...
  for (const r of ROLES) {
    ...
    if (bal < amount) underfunded.push(`  ${r}: ${accounts[r].address} has ${formatUnits(bal, 6)} USDC, needs 10`);
  }
  if (underfunded.length) {
    console.error("Underfunded smart accounts (fund via https://faucet.circle.com, Base Sepolia):\n" + underfunded.join("\n"));
    process.exitCode = 1;
    return;
  }

  // 5. Snapshot ETH (the gas-free claim: this stays flat) and USDC (the revenue split).
  const ethBefore: Record<Role, bigint> = { author: 0n, voucher: 0n, buyer: 0n };
  const usdcBefore: Record<Role, bigint> = { author: 0n, voucher: 0n, buyer: 0n };
  for (const r of ROLES) {
    ethBefore[r] = await publicClient.getBalance({ address: accounts[r].address });
  ...
  const listingId = keccak256(
    encodeAbiParameters(parseAbiParameters("address, bytes32"), [accounts.author.address, skillHash]),
  );
  const approve = (amt: bigint) => ({ to: usdc, abi: erc20Abi, functionName: "approve" as const, args: [av, amt] as const });
  const call = (functionName: string, args: readonly unknown[]) => ({ to: av, abi: agentVouchAbi, functionName, args });
  ...
  async function step(label: string, who: Role, calls: unknown[]) {
    ...
    console.log(
      `  ${label.padEnd(18)} userOp ${hash.slice(0, 10)}… ${receipt.success ? "OK " : "REVERTED"} ` +
  ...
  await step("author.register", "author", [call("registerAgent", ["ipfs://author"])]);
  await step("voucher.register", "voucher", [call("registerAgent", ["ipfs://voucher"])]);
  await step("author.depositBond", "author", [approve(amount), call("depositAuthorBond", [amount])]);
  await step("voucher.vouch", "voucher", [approve(amount), call("vouch", [accounts.author.address, amount])]);
  await step("author.createListing", "author", [
    call("createSkillListing", [skillHash, "ipfs://skill", "Gasless Skill", "Demo skill", amount]),
  ]);
  await step("buyer.purchase", "buyer", [approve(amount), call("purchaseSkill", [listingId])]);
  await step("voucher.claim", "voucher", [call("claimVoucherRevenue", [accounts.author.address])]);
  await step("author.withdraw", "author", [call("withdrawAuthorProceeds", [listingId, 1n, authorShare])]);
  ...
  for (const r of ROLES) {
    const after = await publicClient.getBalance({ address: accounts[r].address });
    const delta = after - ethBefore[r];
    console.log(`  ${r.padEnd(8)} ${formatEther(after)} ETH  (Δ ${formatEther(delta)})  ${delta === 0n ? "✓ zero gas" : ""}`);
  }
  console.log("\nResult — USDC revenue split:");
  for (const r of ROLES) {
    const after = await usdcBalance(accounts[r].address);
    console.log(`  ${r.padEnd(8)} ${formatUnits(after, 6)} USDC  (Δ ${formatUnits(after - usdcBefore[r], 6)})`);
  }
  console.log(
    `\nExpected deltas: buyer -10 (price), voucher -6 (-10 stake +${formatUnits(voucherPool, 6)} pool), ` +
      `author -4 (-10 bond +${formatUnits(authorShare, 6)} proceeds).`,
  );
  console.log(`Total gas sponsored by paymaster: ${formatEther(sponsoredWei)} ETH. Users paid: 0.`);
}
