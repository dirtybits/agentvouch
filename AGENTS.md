...
2. **Node version**: `.nvmrc` says 24, but the agent shell may pin another Node release ahead of nvm. In each Bash call, run `. "$HOME/.nvm/nvm.sh" --no-use && { nvm use --silent || nvm install; }` so sourcing nvm cannot fail early on a missing version, then nvm resolves the Node 24 release named by `.nvmrc`, installing it when necessary. A hard-coded patch-version PATH can silently leave the wrong shell Node active and make Vitest die with `ERR_REQUIRE_ESM`.
...
- **Write for the owner in plain language.** Use the simplified-technical-english skill. Explain the action and result before internal labels such as A1, A4, lifecycle, settlement, or gate. Link readers to `docs/PLAIN_LANGUAGE_GUIDE.md` when several technical terms are needed.
- **Name the exact money-limit rule.** Distinguish "maximum money held at one time" from "maximum money accepted during the whole first release." State whether withdrawals restore capacity. Do not use "exposure cap" or "alpha budget" as a substitute for that explanation.
- **Keep budgets separate.** Customer-money limits, network-fee spending, development cost, and contract code size are different controls. Explain which one is being discussed. Example amounts and written proposals are not approved settings.
- **Keep status and history intact during wording changes.** Preserve filenames, code identifiers, task IDs and statuses, approval records, addresses, transaction evidence, and executable examples. Keep historical decisions identifiable as historical. A language cleanup does not approve implementation, tests, deployments, or real-money actions.

- Name milestones, plan headings, PR titles, and suggested next steps by the work and outcome. Apply the `simplified-technical-english` skill. Use "Automate local buyer-report, stake-deduction, and credit-claim tests", not "Finish the A1 lifecycle executor". Keep sequence codes only as references beside a descriptive name; explain "voucher slashing" as deducting an endorser's staked USDC after an upheld report. Use the names in `docs/ROADMAP.md` and the active plan consistently. Preserve filenames, stable todo IDs, protocol versions, commands, approval records, and historical evidence; a wording change is not a status change or deployment approval.
