...
- **Chain posture (as of 2026-07-06):** Base Sepolia (`eip155:84532`) is the **default** new-user writable path (Phase 8a, PR #74), behind a single rollback env (`NEXT_PUBLIC_AGENTVOUCH_DEFAULT_CHAIN_CONTEXT=solana`, then redeploy). Solana devnet stays fully selectable with its complete trust layer (slashing A1, pause A3 live). The deployed Base contract is the **`base-poc-v0` spike — not mainnet-ready**; Phase 9 is building the Base v1 trust layer; Phase 10 (Base mainnet, `eip155:8453`) is a **BLOCKED gate plan** — any code that enables `eip155:8453` before its checklist passes is a stop-the-line bug.
...
- **Sources of truth** (trust these over prose in handoff messages):
  - Phase/progress record: `.agents/plans/base-port-chain-adapter*.plan.md` frontmatter todos + dated notes, and `gh pr view`.
  - Launch gates + audit findings: `docs/MAINNET_READINESS.md` (Base Mainnet Track section is the active one).
  - Strategy/sequencing: `docs/ROADMAP.md`.
...
4. **GitHub identity**: `gh auth status` — the active account must be `dirtybits`. Session resume can silently flip it to the `andy-s_parafi` EMU account, which cannot create PRs here; fix with `gh auth switch --user dirtybits`. Keep the repo-local Git author on the verified `dirtybits` identity.
...
1. **The mainnet-enable slip.** Enabling `eip155:8453` anywhere before the Phase 10 gate checklist passes. → _Rule: stop-the-line; `getAdapter()` rejecting mainnet is intentional. See §6._
...
5. **The SSR/hydration env mismatch (PR #74 P2).** Reading server-only env in a render-affecting default (`serverValue || clientValue`), so SSR and hydration disagree (#418). → _Rule: render-affecting defaults read only `NEXT_PUBLIC_*` env (`getDefaultChainContext` deliberately ignores server vars). A server-only knob gets a separately-named non-render helper._
...
23. **Known tracked debt (don't "discover" it, don't widen it):** publisher auth (`resolvePublisherAuth`) verifies wallet signatures but not the signed message **scope** on either chain — only raw-download scope-checks. Bind publish auth to a scoped message on both chains before mainnet.
...
30. **Client/server RPC env confusion.** → _Rule: server-side Solana reads use `SOLANA_RPC_URL`; browser/wallet code needs `NEXT_PUBLIC_SOLANA_RPC_URL` (Next.js hides non-`NEXT_PUBLIC_*` from the client). Keep both set to the same endpoint until public reads are fully server-mediated; don't delete either name prematurely._
