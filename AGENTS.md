...
2. **Node version**: `.nvmrc` says 24, but the agent shell may pin Node 20 ahead of nvm. In each Bash call, run `. "$HOME/.nvm/nvm.sh" && nvm use --silent` so nvm resolves the installed Node 24 release named by `.nvmrc`; a hard-coded patch-version PATH can silently leave the older shell Node active and make Vitest die with `ERR_REQUIRE_ESM`.
